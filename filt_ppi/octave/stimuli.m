...
%p_comm_ccw          = 0;
%p_mul_ccw           = 0;
%p_interpolation_factor-1-p_comm_phase
...
%p_interpolation_factor-1-p_comm_phase
  ...
  defines.p_interpolation_factor  = p_interpolation_factor;
  ...
  %% RESPONSE GENERATION
  octave_data=data;
  yy= filter( b, 1, upsample(octave_data, p_interpolation_factor, p_comm_phase)) ;
  %yy= downsample(filter(b,1,octave_data),p_interpolation_factor,p_interpolation_factor-1-p_comm_phase);
